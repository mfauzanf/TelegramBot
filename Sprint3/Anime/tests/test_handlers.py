...
from csuibot.handlers import animeAir, noAnimeParam, animeGroup
...
def test_AnimeNotAvailable(mocker):
    result = "Your Anime cannot be found"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.anime', return_value=result)
    mock_message = Mock(text='/is_airing asdasdsadas')

    animeAir(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == result


def test_AnimeAvailable(mocker):
    fake_text = 'foo bar'
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.anime', return_value=fake_text)
    mock_message = Mock(text='/is_airing Shingeki')
...
def test_NoAnimeParam(mocker):
    result = "Please Insert Anime Name"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers', return_value=result)
    mock_message = Mock(text='/is_airing')

    noAnimeParam(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == result


def test_AnimeGrup(mocker):
    fake_text = "foo bar"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.getAnimeGrup', return_value=fake_text)
    mock_message = Mock(text='hari ini nonton apa?')

    animeGroup(mock_message)

    args, _ = mocked_reply_to.call_args
    assert args[1] == fake_text


def test_AnimeGrupNotAvailable(mocker):
    result = "There's no anime today"
    mocked_reply_to = mocker.patch('csuibot.handlers.bot.reply_to')
    mocker.patch('csuibot.handlers.getAnimeGrup', return_value=result)
    mock_message = Mock(text='hari ini nonton apa?')
