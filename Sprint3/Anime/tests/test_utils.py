...
class TestHipster():
    ...
    def testHipsterGetResult(self):
        jumParagraph = [1, 99]

        for p in jumParagraph:
            countEnter = utils.hipsterGetResult(p).count('\n')
            assert countEnter == p-1


class TestAnime():

    def testAnimeAvailable(self):
        result = utils.anime("Shingeki")
        assert result.count("\n") > 0

    def testAnimeNotAvailable(self):
        result = utils.anime("asdasdsadas")
        assert result == "Your Anime cannot be found"


class TestAnimeGrup():

    def testAnimeGrupAvailable(self):
        result = utils.getAnimeGrup()
        assert result.count("\n") > 0
